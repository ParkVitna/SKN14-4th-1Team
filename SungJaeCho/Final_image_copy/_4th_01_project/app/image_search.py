...
def search_image_google(product_name: str, brand: str) -> str:
    ...
    for item in data.get("items", []):
        ...
        # 개선된 키워드 매칭: 모든 핵심 키워드 중 최소 절반 이상 일치해야 통과
        keyword_matches_count = sum(1 for keyword in meaningful_name_keywords if keyword in filter_target_string)
        required_keyword_count = max(1, len(meaningful_name_keywords) // 2)

        print(f"URL: {image_url}, 타이틀: {item_title}, 브랜드 매치: {brand_match}, 키워드 매치 수: {keyword_matches_count}, 필요 매치 수: {required_keyword_count}")

        if (brand_match and (keyword_matches_count >= required_keyword_count)) or (keyword_matches_count >= required_keyword_count + 1):
            return image_url

    return ""


def get_recommendation_from_web(query: str, cfg, max_retries=2, retry_delay=2):
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            print(f"JSON 파싱 오류 (시도 {attempt}): {e}")
            if attempt == max_retries:
                raise RuntimeError(f"최대 재시도 횟수({max_retries}) 도달: JSON 파싱에 실패했습니다.\n원본 응답: {result['output']}")
            else:
